...
function makeNet(par)
  local ns = {}
  -- The net expects two inputs in a table:
  --  x: the image vector
  --  validActinos: a 0/1 mask for which actions are valid.
  ns.xAndValidActions = nn.Identity()()
  ns.x = nn.SelectTable(1)(ns.xAndValidActions)
  ns.validActions = nn.SelectTable(2)(ns.xAndValidActions)
...
-- The output of our neural net may be length 2 or 3 depending on how many
-- valid actions there are. Here we convert the log probabilities for the
-- valid actions into a full probability distribution over all events
-- (no longer log transformed)
function policyDistr(env)
  local policy = torch.zeros(3)
  policy:maskedCopy(env.validActions, torch.exp(env.logPolicy))
  return policy
end
